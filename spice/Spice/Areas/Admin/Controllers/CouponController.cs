    ...
    public class CouponController : Controller
    {
        ...
        public async Task<IActionResult> Create(Coupon coupons)
        {
            if(ModelState.IsValid)
            {
                ...
                if(files.Count>0)
                {
                    // convert image to a stream of pipes to store in db
                    byte[] p1 = null;   // to convert the image into a byte array
                    using (var fs1 = files[0].OpenReadStream())
                    {
                        using (var ms1 = new MemoryStream())
                        {
                            fs1.CopyTo(ms1);
                            p1 = ms1.ToArray(); // converted image of stream of pipes and store in p1
                        }
                    }
                    coupons.Picture = p1;
                }
