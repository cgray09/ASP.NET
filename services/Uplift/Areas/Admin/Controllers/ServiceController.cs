...
namespace Uplift.Areas.Admin.Controllers
{
    ...
    public class ServiceController : Controller
    {
        ...
        private readonly IWebHostEnvironment _hostEnvironment; // to upload files on the server

        [BindProperty]  // binded from the view 
        ...
        public IActionResult Upsert()
        {
            if (ModelState.IsValid)
            {
                ...
                if (ServVM.Service.Id == 0)
                {
                    ...
                    var uploads = Path.Combine(webRootPath, @"images\services"); // where we upload the image
                ...
                else
                {
                    ...
                    if (files.Count > 0)    // if the service already has images saved
                    {

                        // if were replacing an image 
                        // we delete the image
                        // add the new image
                    ...
                    {
                        // we dont have to do anything but keep the new image that the user submited since there are no others saved
        ...
        {
            // includeProperties: "Category,Frequency" - eager loads Category and Frequency so we can use them
            // when the method is called (since foreign keys in Service model)
