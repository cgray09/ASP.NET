...
namespace AppointmentScheduling.Services
{
    public class AppointmentService : IAppointmentService
    {
        ...
        {
            // the dates in the view model will be strings but DateTime
            // in our model and in the db when saved
            var startDate = DateTime.Parse(model.StartDate);
            var endDate = DateTime.Parse(model.StartDate).AddMinutes(Convert.ToDouble(model.Duration));
            // "Users" is the same table as AspNetUsers in the db (we dont use the AspNet prefix
            // when accessing our tables)
        ...
        public async Task<int> ConfirmEvent(int id)
        {
            ...
            if (appointment != null)
            {
                ...
                return await _db.SaveChangesAsync(); // returns the # of records
                                                     // that were updated
            }
            return 0;
        }
        ...
        public AppointmentVM GetById(int id)
        {
            ...
            }).SingleOrDefault(); // so we get one single appointment
        }
