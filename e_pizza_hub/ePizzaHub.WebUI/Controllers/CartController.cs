    ...
    public class CartController : BaseController
    {
        ...
        Guid CartId
        {
            get
            {
                ...
                {
                    Id = Guid.NewGuid();
                    // if user isnt logged in we'll use CId w/ them
                    // and when one logs in the "CId" is mapped to them
        ...
        // ": base(userAccessor)" is passing userAccessor to CartController
        // parent to child (i think he said you can only go parent to child) 
        // BaseController is the parent (userAccessor is to get logged in user)
        ...
        {
            // he created TempDataExtension.cs to keep the Address from being
            // deleted since temp data is only good for the first redirect
